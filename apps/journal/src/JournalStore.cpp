...
void JournalStore::setHostContext(HostContext *ctx) {
  m_hostCtx = ctx;
  if (m_hostCtx)
...
void JournalStore::ensureLoaded(const QString &nodeId) {
  if ((m_contents.contains(nodeId) || m_titles.contains(nodeId)) ||
      (!m_mgr && !m_hostCtx)) {
    ...
    emit statusChanged(
        nodeId, (m_contents.contains(nodeId) || m_titles.contains(nodeId))
                    ? QString("saved")
                    : QString("unsaved"));
    return;
  }
  ...
  QString query = QString("query($id: String!) { nodes(id: $id) { id journal { "
                          "title content } } }");
  QJsonObject vars;
  vars.insert("id", QJsonValue(nodeId));

  QNetworkReply *reply = nullptr;
  qDebug() << "trying to query" << m_hostCtx->networkAccessManager() << "or"
           << m_mgr;
  if (m_hostCtx) {
    reply = m_hostCtx->graphqlRequest(query, vars);
  } else if (m_mgr) {
    QJsonObject body;
...
void JournalStore::onSaveTimerTimeout() {
  ...
  QNetworkReply *reply = nullptr;
  if (m_hostCtx) {
    reply = m_hostCtx->graphqlRequest(mutation, vars);
  } else if (m_mgr) {
    QJsonObject body;
