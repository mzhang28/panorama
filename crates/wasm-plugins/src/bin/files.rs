...
fn handle(input: &WasmInput) -> WasmOutput {
    match (input.request.method.as_str(), input.endpoint.as_str()) {
        ("POST", "upload") => { let filename = input.request.query_params.get("filename").cloned().unwrap_or_else(|| "unnamed".to_string()); let mime = input.request.query_params.get("mime_type").cloned().unwrap_or_else(|| "application/octet-stream".to_string()); let body_len = input.request.body.as_ref().map(|b| b.len()).unwrap_or(0); let mut fields = HashMap::new(); fields.insert("system:node_title".into(), field_value("String", filename)); fields.insert("files:object_ref".into(), serde_json::json!({"type":"Json","value":{"bucket":"files","key":"test"}})); fields.insert("files:file_size".into(), field_value("Integer", body_len as i64)); fields.insert("files:mime_type".into(), field_value("String", mime)); let mut out = json_ok(serde_json::json!({"uploaded":true,"size":body_len})); out.effects.push(create_effect(fields)); out }
