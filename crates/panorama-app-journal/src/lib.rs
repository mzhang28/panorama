    ...
    Schema {
      ...
      fields: vec![
        ...
        SchemaField {
          ...
          field_type: Some(FieldTypeConstraint {
            type_tag: "Array".into(),
            element_type: Some(Box::new(FieldTypeConstraint {
              type_tag: "NodeRef".into(),
              element_type: None,
            })),
          }),
  ...
  async fn handle_http_request(
    ...
    match (request.method.as_str(), endpoint) {
      ...
      ("POST", "blocks") => {
        ...
        if !tags.is_empty() {
          node.set_field("journal:tags", FieldValue::Json(json!(tags)));
        }
        ...
        let page_id = if let Some(pid) = parent_id {
          if let Some(parent) = ctx.get_node(pid).await? {
            parent
              ...
              .get("journal:page_id")
              .and_then(|v| {
                if let FieldValue::NodeRef(id) = v {
                  Some(*id)
                } else {
                  None
                }
      ...
      // ── GET /blocks/{id} ────────────────────────────────────
      ("GET", _)
        if endpoint.starts_with("blocks/")
      ...
      // ── PUT /blocks/{id} ────────────────────────────────────
      ("PUT", _) if endpoint.starts_with("blocks/") && !endpoint.contains("/children") => {
        let id_str = endpoint.strip_prefix("blocks/").unwrap_or(endpoint);
        let id = Uuid::parse_str(id_str).map_err(|_| PluginError::bad_request("Invalid UUID"))?;
        ...
        if let Some(content) = body.get("content").and_then(|v| v.as_str()) {
          ...
          updates.insert(
            ...
            FieldValue::Array(
              ref_uuids
                .iter()
                .map(|rid| FieldValue::NodeRef(*rid))
                .collect(),
            ),
          );
        }
        ...
        if let Some(props) = body.get("properties") {
          updates.insert(
            "journal:properties".to_string(),
            FieldValue::Json(props.clone()),
          );
        }
