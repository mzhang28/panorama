//! Dashboards Plugin — Full dashboard system for time-series data visualization,
//! matching the four-panel Coding Activity dashboard reference design.
//!
//! ## Architecture
//!
//! ### Dashboard Model
//!
//! Dashboards are stored as nodes with a `dashboards:config` field containing the
//! full dashboard JSON. The model mirrors Dashboards's dashboard JSON structure:
//!
//! ```json
//! {
//!   "uid": "abc123",
...
//!       "queries": [{
//!         "refId": "A",
//!         "dataSource": "io.mzhang.panorama.coding",
...
pub struct PanelQuery {
  /// Which data source plugin to query (e.g., "io.mzhang.panorama.coding")
...
fn default_datasource() -> String {
  "io.mzhang.panorama.coding".to_string()
}
...
/// A data frame in dashboard-compatible format.
...
impl DashboardsPlugin {
  fn time_range_presets() -> Vec<TimeRangePreset> {
    vec![
      ...
      TimeRangePreset {
        label: "Last 3 hours".into(),
        from: "now-3h".into(),
        to: "now".into(),
      },
      TimeRangePreset {
        label: "Last 6 hours".into(),
        from: "now-6h".into(),
        to: "now".into(),
      },
      TimeRangePreset {
        label: "Last 12 hours".into(),
        from: "now-12h".into(),
        to: "now".into(),
      },
      TimeRangePreset {
      ...
      TimeRangePreset {
        label: "Last 1 year".into(),
        from: "now-365d".into(),
        to: "now".into(),
      },
      TimeRangePreset {
        label: "Year to date".into(),
        from: "now/y".into(),
        to: "now".into(),
      },
      TimeRangePreset {
...
impl DashboardsPlugin {
  ...
  async fn execute_data_queries(
    ...
    for query in &request.queries {
      ...
      let ref_id = if query.ref_id.is_empty() {
        "A".to_string()
      } else {
        query.ref_id.clone()
      };
      ...
      let frame = self
        .execute_promql_query(ctx, query, from, to, &ref_id)
        .await?;
      frames.push(frame);
    }
  ...
  async fn execute_promql_query(
    ...
    let registry = promql::registry::MetricRegistry::with_coding_defaults();
...
impl DashboardsPlugin {
  fn dashboard_schema() -> Schema {
    Schema {
      node_id: Uuid::nil(),
      name: "dashboards/Dashboard".to_string(),
      ...
      fields: vec![
        ...
        SchemaField {
          ...
          namespace: "dashboards".to_string(),
          field_type: Some(FieldTypeConstraint {
            type_tag: "Json".into(),
            element_type: None,
          }),
  ...
  fn folder_schema() -> Schema {
    Schema {
      node_id: Uuid::nil(),
      name: "dashboards/Folder".to_string(),
...
impl Plugin for DashboardsPlugin {
  ...
  fn ui_components(&self) -> Vec<UiComponent> {
    vec![
      UiComponent {
        id: "dashboards-main".to_string(),
        name: "Dashboard View".to_string(),
        mount_point: UiMountPoint::MainPage,
        bundle_path: "ui/dashboards.js".to_string(),
      },
      UiComponent {
        id: "dashboards-dashboard".to_string(),
  ...
  fn required_capabilities(&self) -> CapabilityGrants {
    CapabilityGrants {
      ...
      field_write: vec!["dashboards:*".to_string(), "system:node_title".to_string()],
  ...
  async fn handle_http_request(
    ...
    match (request.method.as_str(), endpoint) {
      ...
      // ── Query engine ──────────────────────────────────────
      ("POST", "api/ds/query") => {
        let req: DataQueryRequest = serde_json::from_slice(request.body.as_deref().unwrap_or(&[]))
        ...
        let req = if body_val.get("queries").is_some() {
          ...
          DataQueryRequest {
            queries: vec![PanelQuery {
              data_source: "io.mzhang.panorama.coding".to_string(),
              ref_id: "A".to_string(),
              promql,
              time_range: None,
              limit,
...
impl DashboardsPlugin {
  ...
  async fn find_dashboard_by_uid(
    ...
    let rows = ctx
      .query(
        "MATCH (n) IN space(\"default\") \
                 WHERE HAS_FIELD(n, \"dashboards\", \"uid\") \
                 RETURN n",
      )
      .await?;
    let nodes: Vec<Node> = rows
    ...
    for node in &nodes {
      if let Some(FieldValue::String(node_uid)) = node.get_field("dashboards:uid") {
        if node_uid == uid {
          if let Some(FieldValue::Json(config)) = node.get_field("dashboards:config") {
  ...
  async fn find_dashboard_node(
    ...
    Ok(
      nodes
        .into_iter()
        .find(|n| matches!(n.get_field("dashboards:uid"), Some(FieldValue::String(u)) if u == uid)),
    )
  }
  ...
  async fn handle_list_dashboards(
    ...
    let dashboards: Vec<serde_json::Value> = nodes
      .iter()
      .filter_map(|n| {
        let uid = match n.get_field("dashboards:uid") {
          Some(FieldValue::String(s)) => s.clone(),
          _ => return None,
        };
        let title = n.title().unwrap_or("Untitled");
        let tags = match n.get_field("dashboards:tags") {
  ...
  async fn handle_create_dashboard(
    ...
    node.set_field("dashboards:uid", FieldValue::String(dashboard.uid.clone()));
    node.set_field(
      "dashboards:config",
      FieldValue::Json(
        serde_json::to_value(&dashboard)
          .map_err(|e| PluginError::internal(format!("Failed to serialize dashboard: {}", e)))?,
      ),
    );
    node.set_field("dashboards:version", FieldValue::Integer(1));
  ...
  async fn handle_update_dashboard(
    ...
    let dashboard: Dashboard = serde_json::from_slice(request.body.as_deref().unwrap_or(&[]))
      .map_err(|e| PluginError::bad_request(&format!("Invalid dashboard JSON: {}", e)))?;

    let current_version = match existing.get_field("dashboards:version") {
    ...
    updates.insert(
      "dashboards:uid".to_string(),
      FieldValue::String(uid.to_string()),
    );
    updates.insert(
      "dashboards:config".to_string(),
      FieldValue::Json(
        serde_json::to_value(&dashboard)
    ...
    updates.insert(
      "dashboards:version".to_string(),
      FieldValue::Integer(current_version + 1),
    );
    if !dashboard.tags.is_empty() {
      updates.insert(
        "dashboards:tags".to_string(),
        FieldValue::String(dashboard.tags.join(", ")),
      );
    }
  ...
  async fn handle_delete_dashboard(
    ...
    ctx
      .log(
        LogLevel::Info,
        &format!("Deleted dashboard: {} ({})", title, uid),
      )
      .await;
  ...
  async fn handle_get_home_dashboard(
    ...
    for node in &nodes {
      if let Some(FieldValue::String(uid)) = node.get_field("dashboards:uid") {
        if uid == "home" {
          if let Some(FieldValue::Json(config)) = node.get_field("dashboards:config") {
            if let Ok(dashboard) = serde_json::from_value::<Dashboard>(config.clone()) {
    ...
    node.set_field("dashboards:uid", FieldValue::String("home".to_string()));
    node.set_field(
      "dashboards:config",
      FieldValue::Json(
    ...
    node.set_field("dashboards:version", FieldValue::Integer(1));
    node.set_field("dashboards:tags", FieldValue::String("home".to_string()));
  ...
  async fn handle_list_folders(
    ...
    let folders: Vec<serde_json::Value> = nodes
      .iter()
      .filter(|n| match n.get_field("dashboards:config") {
        None => true,
  ...
  async fn handle_create_folder(
    ...
    let uid = body["uid"]
      .as_str()
      .map(String::from)
      .unwrap_or_else(|| Self::generate_uid());

    let mut node = Node::new(Uuid::nil());
    node.set_field("system:node_title", FieldValue::String(title.to_string()));
    node.set_field("dashboards:uid", FieldValue::String(uid.clone()));
    ...
    panels: vec![
      ...
      Panel {
        ...
        queries: vec![PanelQuery {
          data_source: "io.mzhang.panorama.coding".to_string(),
          ref_id: "A".to_string(),
          promql: Some("sum by (project) (coding_duration)".to_string()),
          time_range: None,
          limit: 10,
          hide: false,
        }],
      ...
      Panel {
        ...
        queries: vec![PanelQuery {
          data_source: "io.mzhang.panorama.coding".to_string(),
          ref_id: "A".to_string(),
          promql: Some("sum by (language) (coding_duration)".to_string()),
          time_range: None,
          limit: 10,
          hide: false,
        }],
      ...
      Panel {
        ...
        queries: vec![PanelQuery {
          data_source: "io.mzhang.panorama.coding".to_string(),
          ref_id: "A".to_string(),
          promql: Some("topk(15, sum by (entity) (coding_duration))".to_string()),
          time_range: None,
          limit: 15,
          hide: false,
        }],
...
mod tests {
  ...
  fn test_resolve_time_now() {
    ...
    let resolved = DashboardsPlugin::resolve_time("now");
  ...
  fn test_resolve_time_relative_days() {
    let now = Utc::now();
    let resolved = DashboardsPlugin::resolve_time("now-7d");
  ...
  fn test_resolve_time_relative_hours() {
    let now = Utc::now();
    let resolved = DashboardsPlugin::resolve_time("now-24h");
    let diff = now - resolved;
  ...
  fn test_generate_uid() {
    let uid1 = DashboardsPlugin::generate_uid();
    let uid2 = DashboardsPlugin::generate_uid();
  ...
  fn test_time_range_presets() {
    let presets = DashboardsPlugin::time_range_presets();
