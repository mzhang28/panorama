...
impl DashboardsPlugin {
    ...
    async fn find_dashboard_node(
        ...
        Ok(nodes.into_iter().find(|n| {
            matches!(n.get_field("dashboards:uid"), Some(FieldValue::String(u)) if u == uid)
        }))
    }
    ...
    async fn handle_update_dashboard(
        ...
        updates.insert("dashboards:uid".to_string(), FieldValue::String(uid.to_string()));
        updates.insert(
        ...
        updates.insert("dashboards:version".to_string(), FieldValue::Integer(current_version + 1));
        if !dashboard.tags.is_empty() {
            updates.insert(
    ...
    async fn handle_list_folders(
        ...
        let folders: Vec<serde_json::Value> = nodes
            .iter()
            .filter(|n| {
                match n.get_field("dashboards:config") {
                    None => true,
