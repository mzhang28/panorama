...
//! ## Example
//!
//! `rate(coding_duration{project="panorama"}[5m])` translates to:
//! - PQL: `MATCH (n) IN space("default") WHERE HAS_FIELD(n, "coding", "duration")
//!          AND n.coding.project = "panorama" RETURN n`
...
pub struct TranslatedQuery {
  ...
  /// The label fields to extract from each node (fully qualified, e.g. "coding:project").
  pub label_fields: Vec<String>,
  /// The value field key (e.g. "coding:duration").
  pub value_field: String,
  /// The time field key (e.g. "coding:time").
...
fn translate_expr(expr: &Expr, ctx: &Ctx) -> Result<TranslatedQuery, String> {
  match expr {
    ...
    Expr::StringLiteral(_) => {
      Err("string literals are not supported as standalone expressions".into())
    }
...
fn translate_instant_vector(
  ...
  // Required field presence (e.g., HAS_FIELD(n, "coding", "entity"))
  for req_field in &mapping.required_fields {
    if let Some((ns, field)) = split_field_key(req_field) {
      predicates.push(format!("HAS_FIELD(n, \"{}\", \"{}\")", ns, field));
    }
  }
...
fn translate_range_vector(rv: &RangeVector, ctx: &Ctx) -> Result<TranslatedQuery, String> {
  translate_instant_vector(&rv.vector, Some(&rv.range), ctx)
}

// ── Functions ─────────────────────────────────────────────────────────────────

fn translate_function(fc: &FunctionCall, ctx: &Ctx) -> Result<TranslatedQuery, String> {
  match fc.name.to_lowercase().as_str() {
    ...
    "count" => {
      ...
      let mut tq = translate_expr(&fc.args[0], ctx)?;
      tq.post_steps
        .push(PostStep::GroupCount { group_by: vec![] });
      Ok(tq)
    }
    "sort" => {
      if fc.args.is_empty() {
    ...
    "changes" | "resets" | "deriv" | "predict_linear" | "holt_winters" | "idelta"
    | "label_replace" | "label_join" | "timestamp" | "vector" | "scalar" | "time"
    | "days_in_month" | "day_of_month" | "day_of_week" | "hour" | "minute" | "month" | "year" => {
...
fn translate_aggregation(agg: &Aggregation, ctx: &Ctx) -> Result<TranslatedQuery, String> {
  ...
  let step = match agg.op {
    AggregationOp::Sum => PostStep::GroupSum {
      group_by: group_by.clone(),
    },
    AggregationOp::Avg => PostStep::GroupAvg {
      group_by: group_by.clone(),
    },
    AggregationOp::Min => PostStep::GroupMin {
      group_by: group_by.clone(),
    },
    AggregationOp::Max => PostStep::GroupMax {
      group_by: group_by.clone(),
    },
    AggregationOp::Count => PostStep::GroupCount {
      group_by: group_by.clone(),
    },
    AggregationOp::Stddev => PostStep::GroupStddev {
      group_by: group_by.clone(),
    },
    AggregationOp::Stdvar => PostStep::GroupStdvar {
      group_by: group_by.clone(),
    },
    AggregationOp::TopK => {
      let k = agg
...
fn translate_binary_op(bin: &BinaryOp, ctx: &Ctx) -> Result<TranslatedQuery, String> {
...
fn translate_unary(un: &UnaryOp, ctx: &Ctx) -> Result<TranslatedQuery, String> {
...
fn translate_subquery(sq: &Subquery, _ctx: &Ctx) -> Result<TranslatedQuery, String> {
...
fn resolve_metric<'a>(
  metric_name: &str,
  registry: &'a MetricRegistry,
) -> Result<&'a MetricMapping, String> {
  if let Some(mapping) = registry.get(metric_name) {
    return Ok(mapping);
  }

  // Fallback: try convention-based parsing
  // "coding_duration" → namespace="coding", value_field="duration"
  Err(format!(
...
/// Split a field key like "coding:entity" into ("coding", "entity").
fn split_field_key(key: &str) -> Option<(&str, &str)> {
  key.split_once(':')
}

/// Convert a colon-separated field key like "coding:time" to PQL dot notation
/// like "coding.time".
...
fn nodes_to_datapoints(
  ...
  for node in nodes {
    ...
    let timestamp = match node.get_field(time_field) {
      Some(FieldValue::Float(f)) => *f,
      Some(FieldValue::Integer(i)) => *i as f64,
      Some(FieldValue::DateTime(s)) => chrono::DateTime::parse_from_rfc3339(&s)
  ...
  match step {
    ...
    PostStep::GroupAvg { group_by } => {
      ...
      for (key, series) in grouped {
        ...
        let keys_to_keep: Vec<String> = labels
          .keys()
          .filter(|k| {
            group_by.is_empty()
              || group_by
                .iter()
                .any(|gb| gb.ends_with(&format!(":{}", k)) || *gb == **k)
          })
          .cloned()
          .collect();
    ...
    PostStep::GroupMin { group_by } => aggregate_group_minmax(&points, group_by, |a, b| a < b),
    PostStep::GroupMax { group_by } => aggregate_group_minmax(&points, group_by, |a, b| a > b),
    ...
    PostStep::HistogramQuantile { quantile } => {
      ...
      let mut buckets: Vec<(f64, f64)> = points
        .iter()
        .map(|p| {
          let le = p
            .labels
    ...
    let key: BTreeMap<String, String> = if group_by.is_empty() {
      ...
      point
        ...
        .filter(|(k, _)| {
          ...
          group_by
            .iter()
            .any(|gb| gb.ends_with(&format!(":{}", k)) || *gb == **k || fq == *gb)
        })
...
fn aggregate_group_minmax(
  ...
  for (key, series) in grouped {
    ...
    let best = series.iter().fold(f64::NAN, |best, p| {
      if best.is_nan() || better(p.value, best) {
        p.value
      } else {
        best
      }
...
/// Convert DataPoints to a dashboard-compatible DataFrame.
fn datapoints_to_dataframe(points: &[DataPoint], name: &str) -> Result<crate::DataFrame, String> {
  use serde_json::json;
  ...
  for point in sorted_points {
    let mut row = vec![json!(point.timestamp), json!(point.value)];
...
mod tests {
  ...
  fn test_registry() -> MetricRegistry {
    MetricRegistry::with_coding_defaults()
  }

  fn translate_str(promql: &str) -> Result<TranslatedQuery, String> {
    let expr = super::super::parser::parse(promql).map_err(|e| e.to_string())?;
  ...
  fn test_simple_metric() {
    let tq = translate_str("coding_duration").unwrap();
    assert!(tq.pql.contains("MATCH (n) IN space(\"default\")"));
    assert!(tq.pql.contains("HAS_FIELD(n, \"coding\", \"duration\")"));
    assert_eq!(tq.value_field, "coding:duration");
  }

  #[test]
  fn test_metric_with_labels() {
    let tq = translate_str(r#"coding_duration{project="panorama"}"#).unwrap();
    assert!(tq.pql.contains("n.coding.project = \"panorama\""));
    assert!(tq.label_fields.contains(&"coding:project".to_string()));
  }

  #[test]
  fn test_range_vector() {
    let tq = translate_str("coding_duration[5m]").unwrap();
  ...
  #[test]
  fn test_rate() {
    let tq = translate_str("rate(coding_duration[5m])").unwrap();
    assert!(tq.pql.contains("HAS_FIELD"));
  ...
  #[test]
  fn test_sum_by() {
    let tq = translate_str("sum by (project) (rate(coding_duration[5m]))").unwrap();
