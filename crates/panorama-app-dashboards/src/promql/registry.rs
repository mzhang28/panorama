...
pub struct MetricMapping {
  /// The Prometheus metric name (e.g., "coding_duration").
  pub metric_name: String,
  /// The Panorama namespace (e.g., "coding").
  ...
  /// part of this metric (e.g., ["coding:entity"]).
  #[serde(default)]
...
impl MetricRegistry {
  ...
  pub fn infer_metric_name(&self, namespace: &str, value_field: &str) -> Option<String> {
    ...
    if matches!(namespace, "coding" | "restaurants") {
      return Some(format!("{}_{}", namespace, value_field));
    }
    None
  }

  /// Create a registry with default Coding Activity metric mappings.
  pub fn with_coding_defaults() -> Self {
    let mut registry = Self::new();

    // Coding Activity duration metric
    registry.register(MetricMapping {
      metric_name: "coding_duration".to_string(),
      namespace: "coding".to_string(),
      ...
      required_fields: vec!["coding:entity".to_string()],
      default_labels: HashMap::from([("source".to_string(), "coding".to_string())]),
      description: "Coding duration in seconds from Coding Activity heartbeats".to_string(),
    });
