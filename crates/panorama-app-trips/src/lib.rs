    ...
    match (request.method.as_str(), endpoint) {
      ...
      ("POST", "events") => {
        ...
        if let Some(loc) = body["location_name"].as_str() {
          node.set_field("trips:location_name", FieldValue::String(loc.to_string()));
        }
      ...
      ("GET", "events") => {
        ...
        let rows = ctx
          .query(
            "MATCH (n) IN space(\"default\") WHERE HAS_FIELD(n, \"trips\", \"trip_id\") RETURN n",
          )
          .await?;
        ...
        nodes.sort_by(|a, b| {
          let ta = a.get_field("system:node_start_time").and_then(|v| match v {
            FieldValue::DateTime(s) => Some(s.clone()),
            _ => None,
          });
          let tb = b.get_field("system:node_start_time").and_then(|v| match v {
            FieldValue::DateTime(s) => Some(s.clone()),
            _ => None,
          });
          ta.cmp(&tb)
      ...
      ("GET", "events/map") => {
        let rows = ctx
          .query(
            "MATCH (n) IN space(\"default\") WHERE HAS_FIELD(n, \"trips\", \"latitude\") RETURN n",
          )
          .await?;
        ...
        let map_events: Vec<_> = nodes
          .into_iter()
          .filter(|n| {
            n.get_field("trips:latitude").is_some() && n.get_field("trips:longitude").is_some()
          })
          .map(|n| {
            serde_json::json!({
                "id": n.id.to_string(),
