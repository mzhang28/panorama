...
export default function SubsonicApp({ pluginId }: SubsonicAppProps) {
  ...
  const fetchLibrary = async () => {
    ...
    try {
      ...
      setArtists(
        aData['subsonic-response']?.artists?.index?.[0]?.artist || [],
      )
        ...
        <div className="card" style={{ marginBottom: 16 }}>
          ...
          >
            <span>
              Server:{' '}
              <strong>
                {pingResult['subsonic-response']?.serverVersion ||
                  'unknown'}
              </strong>
            </span>
            <span className="text-muted">
              Type: {pingResult['subsonic-response']?.type || 'unknown'}
            </span>
          </div>
        </div>
      )}
