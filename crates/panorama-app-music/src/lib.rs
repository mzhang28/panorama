//! music-streaming-compatible Music Streaming App.
//! Demonstrates from DESIGN.md: external protocol compatibility, streaming, object storage.
//!
//! Workflow:
//! - Expose Music Library API endpoints (ping, getArtists, getAlbumList2, stream) for music clients
...
impl Plugin for MusicPlugin {
  ...
  fn description(&self) -> &str {
    "music-streaming-compatible music streaming interface"
  }
  ...
  fn http_endpoints(&self) -> Vec<HttpEndpoint> {
    vec![
      HttpEndpoint {
        ...
        description: "Music Library ping endpoint for client compatibility".to_string(),
      },
      HttpEndpoint {
        method: HttpMethod::GET,
  ...
  fn ui_components(&self) -> Vec<UiComponent> {
    vec![UiComponent {
      id: "music-library".to_string(),
      name: "Music Library".to_string(),
      mount_point: UiMountPoint::MainPage,
      bundle_path: "ui/music.js".to_string(),
    }]
  }

  fn required_capabilities(&self) -> CapabilityGrants {
    CapabilityGrants {
      field_read: vec!["music:*".to_string(), "system:node_title".to_string()],
      field_write: vec!["music:*".to_string(), "system:node_title".to_string()],
  ...
  async fn handle_http_request(
    ...
    match (request.method.as_str(), endpoint) {
      ("GET", "rest/ping") => {
        // music-streaming-compatible ping response
      ...
      ("GET", "rest/getArtists") => {
        ...
        let artists: Vec<_> = nodes
          .iter()
          .filter(|n| {
            n.get_field("music:artist_id").is_none()
              && n.get_field("music:album_id").is_none()
              && n.get_field("music:audio_ref").is_none()
              && n.title().is_some()
          })
          .map(|n| {
            serde_json::json!({
      ...
      ("GET", "rest/getAlbumList2") => {
        let rows = ctx
          .query(
            "MATCH (n) IN space(\"default\") WHERE HAS_FIELD(n, \"music\", \"artist_id\") RETURN n",
          )
          .await?;
        let nodes: Vec<Node> = rows
          .iter()
        ...
        let albums: Vec<_> = nodes
          .iter()
          .filter(|n| {
            n.get_field("music:artist_id").is_some() && n.get_field("music:audio_ref").is_none()
          })
          .map(|n| {
            serde_json::json!({
                "id": n.id.to_string(),
                "name": n.title().unwrap_or("Unknown"),
                "artistId": match n.get_field("music:artist_id") {
      ...
      ("GET", "rest/stream") => {
        ...
        let id =
          Uuid::parse_str(track_id).map_err(|_| PluginError::bad_request("Invalid track ID"))?;
        let node = ctx
          .get_node(id)
          .await?
        ...
        if let Some(FieldValue::ObjectRef(obj_ref)) = node.get_field("music:audio_ref") {
          let obj = ctx
      ...
      ("POST", "upload") => {
        ...
        let obj_ref = ctx
          .put_object("music-audio", &filename, body, "audio/mpeg")
          .await?;

        // Create track node
        let mut node = Node::new(Uuid::nil());
        node.set_field("system:node_title", FieldValue::String(title));
        node.set_field("music:audio_ref", FieldValue::ObjectRef(obj_ref));
