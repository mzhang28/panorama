...
use crate::reactor::{
  ...
  ReactorStatus, ReactorTrigger, WatchScope, WatchTrigger,
};
...
pub enum EvalHookResult {
  ...
  Rejected { reason: String, reactor_id: Uuid },
...
fn hooks_match(actual: &HookPoint, registered: &HookPoint) -> bool {
  match (actual, registered) {
    ...
    (
      ...
      (a_field == r_field || r_field.is_empty()) && scope_compatible(a_scope, r_scope)
    }
    (
      HookPoint::BeforeNodeDelete {
        scope_schema_id: a_scope,
      },
      HookPoint::BeforeNodeDelete {
        scope_schema_id: r_scope,
    ...
    (
      HookPoint::BeforeSchemaInstall {
        scope_schema_id: a_schema,
      },
      HookPoint::BeforeSchemaInstall {
        scope_schema_id: r_schema,
      },
    ) => a_schema == r_schema,
    (
      HookPoint::BeforeSchemaMigrate {
        scope_schema_id: a_schema,
      },
      HookPoint::BeforeSchemaMigrate {
        scope_schema_id: r_schema,
...
pub fn eval_deferred_dispatch(reactors: &[Reactor], entries: &[OpStreamEntry]) -> Vec<(Uuid, u64)> {
...
pub fn eval_backoff(attempt: u32, base_ms: u64, max_ms: u64, multiplier: f64) -> u64 {
...
mod tests {
  ...
  fn test_no_cycle_in_linear_chain() {
    let existing: Vec<(String, String)> = vec![("A".into(), "B".into()), ("B".into(), "C".into())];
  ...
  fn test_simple_cycle_detected() {
    let existing: Vec<(String, String)> = vec![("A".into(), "B".into())];
