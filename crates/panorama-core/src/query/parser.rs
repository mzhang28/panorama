...
fn parse_value(p: &mut Parser) -> Result<Value, ParseError> {
  ...
  match p.peek() {
    ...
    Some('f') | Some('F') => {
      if peek_keyword(p, "false") {
        p.expect_keyword("false")?;
        Ok(Value::Boolean(false))
      } else {
        p.err("expected 'false'")
      }
    }
    Some('n') | Some('N') => {
      if peek_keyword(p, "null") {
        p.expect_keyword("null")?;
        Ok(Value::Null)
      } else {
        p.err("expected 'null'")
