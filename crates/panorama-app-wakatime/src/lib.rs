//! WakaTime Plugin — Full WakaTime API-compatible heartbeat collection,
//! duration computation, and multi-dimensional stats engine.
//!
//! ## Architecture
//!
//! This plugin implements the complete [WakaTime v1 Heartbeat API](
//! https://wakatime.com/developers#heartbeats) for both read and write
//! operations as well as status/stats endpoints.
//!
//! ### Data Model
//!
//! Three node schemas:
//! - **Heartbeat** — raw editor heartbeat with all 25+ WakaTime API fields
...
//!
//! Endpoints mirror the upstream API so existing WakaTime editor plugins and
//! the `wakatime-cli` can submit heartbeats without modification:
//! - `POST /users/current/heartbeats` (WakaTime-compatible path)
...
impl WakatimePlugin {
    /// Full WakaTime heartbeat schema covering every field in the upstream API
    ...
    fn heartbeat_schema() -> Schema {
        Schema {
            ...
            fields: vec![
                // ── System fields ──────────────────────────────────────
                schema_field("node_time", "system", true, "When the heartbeat was recorded"),
                // ── Core WakaTime fields ───────────────────────────────
                schema_field("entity", "wakatime", true, "File path, domain, or app identifier"),
                schema_field("type", "wakatime", true, "Entity type: file, app, url, or domain"),
                schema_field("category", "wakatime", false, "Activity category (coding, building, debugging, etc.)"),
                schema_field("time", "wakatime", true, "UNIX epoch timestamp with fractional seconds"),
                schema_field("project", "wakatime", false, "Project name"),
                schema_field("project_root_count", "wakatime", false, "Number of folders in project root path"),
                schema_field("branch", "wakatime", false, "Git branch name"),
                schema_field("language", "wakatime", false, "Programming language name"),
                schema_field("dependencies", "wakatime", false, "Comma-separated dependency list"),
                schema_field("machine_name_id", "wakatime", false, "Unique machine identifier"),
                // ── AI / GenAI fields ──────────────────────────────────
                schema_field("ai_line_changes", "wakatime", false, "Lines added/removed by GenAI since last heartbeat"),
                schema_field("human_line_changes", "wakatime", false, "Lines added/removed by manual typing since last heartbeat"),
                schema_field("ai_session", "wakatime", false, "AI session identifier"),
                schema_field("ai_input_tokens", "wakatime", false, "User input tokens consumed since last heartbeat"),
                schema_field("ai_output_tokens", "wakatime", false, "Model output tokens consumed since last heartbeat"),
                schema_field("ai_prompt_length", "wakatime", false, "Prompt characters typed to AI since last heartbeat"),
                schema_field("ai_subscription_plan", "wakatime", false, "GenAI tool subscription plan tier"),
                // ── Editor position fields ─────────────────────────────
                schema_field("lines", "wakatime", false, "Total lines in the entity file"),
                schema_field("lineno", "wakatime", false, "Current cursor line number (1-based)"),
                schema_field("cursorpos", "wakatime", false, "Current cursor column position (1-based)"),
                schema_field("is_write", "wakatime", false, "Whether this heartbeat was triggered by a write"),
                // ── Computed fields (set by this plugin, not submitted by clients) ─
                schema_field("duration", "wakatime", false, "Duration in seconds since previous heartbeat"),
                schema_field("is_ai_generated", "wakatime", false, "Whether this heartbeat involved AI code generation"),
            ],
    ...
    fn duration_schema() -> Schema {
        Schema {
            node_id: Uuid::nil(),
            name: "wakatime/Duration".to_string(),
            version: SchemaVersion::new(1, 0),
            fields: vec![
                schema_field("node_time", "system", true, "Start time of the coding session"),
                schema_field("node_end_time", "system", true, "End time of the coding session"),
                schema_field("entity", "wakatime", true, "Primary entity (file/domain)"),
                schema_field("project", "wakatime", false, "Project name"),
                schema_field("language", "wakatime", false, "Programming language"),
                schema_field("category", "wakatime", false, "Activity category"),
                schema_field("duration_seconds", "wakatime", true, "Total duration in seconds"),
                schema_field("heartbeat_count", "wakatime", false, "Number of heartbeats in this session"),
                schema_field("machine_name_id", "wakatime", false, "Machine that produced this session"),
            ],
            schema_mode: SchemaMode::Preferred,
            previous_versions: vec![],
            migrations: vec![],
    ...
    fn daily_summary_schema() -> Schema {
        Schema {
            node_id: Uuid::nil(),
            name: "wakatime/DailySummary".to_string(),
            version: SchemaVersion::new(1, 0),
            fields: vec![
                schema_field("node_time", "system", true, "The date this summary covers"),
                schema_field("date", "wakatime", true, "ISO date string (YYYY-MM-DD)"),
                schema_field("project", "wakatime", false, "Project name"),
                schema_field("language", "wakatime", false, "Programming language"),
                schema_field("entity", "wakatime", false, "File path / entity"),
                schema_field("category", "wakatime", false, "Activity category"),
                schema_field("total_seconds", "wakatime", true, "Total coding seconds for this dimension"),
                schema_field("heartbeat_count", "wakatime", false, "Number of heartbeats"),
                schema_field("session_count", "wakatime", false, "Number of coding sessions"),
                schema_field("ai_seconds", "wakatime", false, "Seconds involving AI-generated code"),
                schema_field("lines_added", "wakatime", false, "Total human lines added"),
                schema_field("lines_removed", "wakatime", false, "Total human lines removed"),
            ],
            schema_mode: SchemaMode::Preferred,
            previous_versions: vec![],
            migrations: vec![],
...
impl WakatimePlugin {
    ...
    async fn fetch_heartbeats_in_range(
        ...
        let rows = ctx
            .query("MATCH (n) IN space(\"default\") WHERE HAS_FIELD(n, \"wakatime\", \"entity\") RETURN n ORDER BY n.system.node_time ASC")
            .await?;
        let all_nodes: Vec<Node> = rows.iter().filter_map(panorama_core::query::row_to_node).collect();
    ...
    fn compute_timeseries(
        ...
        let result: Vec<serde_json::Value> = series
            ...
            .map(|(key, mut points)| {
                ...
                // Fill gaps
                let filled = self.fill_time_gaps(&points, start.timestamp(), end.timestamp(), interval_secs);
...
fn node_time_epoch(node: &Node) -> f64 {
    // Try wakatime:time (the raw heartbeat timestamp) first
...
fn heartbeat_is_ai(hb: &serde_json::Value) -> bool {
    ...
    hb.get("ai_session").and_then(|v| v.as_str()).map(|s| !s.is_empty()).unwrap_or(false)
        || hb.get("ai_line_changes").and_then(|v| v.as_i64()).map(|n| n > 0).unwrap_or(false)
        || hb.get("ai_input_tokens").and_then(|v| v.as_i64()).map(|n| n > 0).unwrap_or(false)
        || hb.get("ai_output_tokens").and_then(|v| v.as_i64()).map(|n| n > 0).unwrap_or(false)
}
...
impl Plugin for WakatimePlugin {
    ...
    fn description(&self) -> &str {
        "Full WakaTime API-compatible heartbeat collection with stats, durations, and leaderboards"
    }
    ...
    fn http_endpoints(&self) -> Vec<HttpEndpoint> {
        vec![
            // ── WakaTime-compatible paths ──────────────────────────
            HttpEndpoint {
                method: HttpMethod::POST,
                path: "/users/current/heartbeats".to_string(),
                description: "WakaTime API: receive a single heartbeat".to_string(),
            },
            HttpEndpoint {
                method: HttpMethod::POST,
                path: "/users/current/heartbeats.bulk".to_string(),
                description: "WakaTime API: receive bulk heartbeats (max 25)".to_string(),
            },
            HttpEndpoint {
                method: HttpMethod::GET,
                path: "/users/current/heartbeats".to_string(),
                description: "WakaTime API: get heartbeats for a date".to_string(),
            },
            HttpEndpoint {
                method: HttpMethod::GET,
                path: "/users/current/durations".to_string(),
                description: "WakaTime API: get durations for a date".to_string(),
            },
            HttpEndpoint {
                method: HttpMethod::DELETE,
                path: "/users/current/heartbeats.bulk".to_string(),
                description: "WakaTime API: delete heartbeats for a date".to_string(),
            },
            // ── Convenience paths ──────────────────────────────────
            HttpEndpoint {
                method: HttpMethod::POST,
    ...
    async fn handle_http_request(
        ...
        match (request.method.as_str(), endpoint) {
            // ── POST heartbeat(s) — WakaTime-compatible ────────────
            ...
            ("GET", "stats") => {
                let query = StatsQuery {
                    range: request.query_params.get("range").cloned().unwrap_or_default(),
                    group_by: request.query_params.get("group_by").cloned(),
                    sub_group_by: request.query_params.get("sub_group_by").cloned(),
                    aggregation: request.query_params.get("aggregation").cloned().unwrap_or_default(),
                    filter: request.query_params.get("filter").cloned(),
                    bucket: request.query_params.get("bucket").cloned().unwrap_or_default(),
                    limit: request
                        .query_params
                        .get("limit")
            ...
            ("POST", "stats") => {
                let query: StatsQuery = serde_json::from_slice(
            ...
            ("GET", "summaries") => {
                ...
                let rows = ctx
                    .query("MATCH (n) IN space(\"default\") WHERE HAS_FIELD(n, \"wakatime\", \"total_seconds\") RETURN n ORDER BY n.system.node_time DESC")
                    .await?;
                let mut summaries: Vec<Node> =
                    rows.iter().filter_map(panorama_core::query::row_to_node).collect();
                let (start, end) = parse_time_range(&range);
...
impl WakatimePlugin {
    ...
    /// This is the primary write path — compatible with the upstream WakaTime
    /// API so existing editor plugins and `wakatime-cli` can submit data.
    async fn handle_post_heartbeats(
        ...
        // The WakaTime API spec says: bulk endpoint accepts array, single
        ...
        let heartbeats: Vec<serde_json::Value> = if body.is_array() {
            ...
            // Enforce bulk limit of 25 per WakaTime spec
            if arr.len() > 25 {
    ...
    async fn create_heartbeat_node(
        ...
        node.set_field("wakatime:type", FieldValue::String(hb_type.to_string()));

        let time = hb["time"].as_f64().unwrap_or_else(|| {
            Utc::now().timestamp() as f64
        });
        node.set_field("wakatime:time", FieldValue::Float(time));
        node.set_field(
            "system:node_time",
            FieldValue::DateTime(
        ...
        if let Some(v) = hb["category"].as_str() {
            node.set_field("wakatime:category", FieldValue::String(v.to_string()));
        }
        if let Some(v) = hb["project"].as_str() {
            node.set_field("wakatime:project", FieldValue::String(v.to_string()));
        }
        if let Some(v) = hb["project_root_count"].as_i64() {
            node.set_field("wakatime:project_root_count", FieldValue::Integer(v));
        }
        if let Some(v) = hb["branch"].as_str() {
            node.set_field("wakatime:branch", FieldValue::String(v.to_string()));
        }
        if let Some(v) = hb["language"].as_str() {
            node.set_field("wakatime:language", FieldValue::String(v.to_string()));
        }
        if let Some(v) = hb["dependencies"].as_str() {
            node.set_field("wakatime:dependencies", FieldValue::String(v.to_string()));
        }
        if let Some(v) = hb["machine_name_id"].as_str() {
            node.set_field(
                "wakatime:machine_name_id",
        ...
        if let Some(v) = hb["ai_line_changes"].as_i64() {
            node.set_field("wakatime:ai_line_changes", FieldValue::Integer(v));
        }
        if let Some(v) = hb["human_line_changes"].as_i64() {
            node.set_field("wakatime:human_line_changes", FieldValue::Integer(v));
        }
        if let Some(v) = hb["ai_session"].as_str() {
            node.set_field("wakatime:ai_session", FieldValue::String(v.to_string()));
        }
        if let Some(v) = hb["ai_input_tokens"].as_i64() {
            node.set_field("wakatime:ai_input_tokens", FieldValue::Integer(v));
        }
        if let Some(v) = hb["ai_output_tokens"].as_i64() {
            node.set_field("wakatime:ai_output_tokens", FieldValue::Integer(v));
        }
        if let Some(v) = hb["ai_prompt_length"].as_i64() {
            node.set_field("wakatime:ai_prompt_length", FieldValue::Integer(v));
        }
        if let Some(v) = hb["ai_subscription_plan"].as_str() {
            node.set_field(
                "wakatime:ai_subscription_plan",
                FieldValue::String(v.to_string()),
            );
        }
        ...
        if let Some(v) = hb["lines"].as_i64() {
            node.set_field("wakatime:lines", FieldValue::Integer(v));
        }
        if let Some(v) = hb["lineno"].as_i64() {
            node.set_field("wakatime:lineno", FieldValue::Integer(v));
        }
        if let Some(v) = hb["cursorpos"].as_i64() {
            node.set_field("wakatime:cursorpos", FieldValue::Integer(v));
        }
        if let Some(v) = hb["is_write"].as_bool() {
            node.set_field("wakatime:is_write", FieldValue::Boolean(v));
        }

        // ── Computed fields ────────────────────────────────────────
        // Duration from the heartbeat payload if provided
        if let Some(v) = hb["duration"].as_f64() {
            node.set_field("wakatime:duration", FieldValue::Float(v));
        }
        // AI flag
        node.set_field(
            "wakatime:is_ai_generated",
    ...
    /// Convert a stored heartbeat node back to WakaTime API-compatible JSON.
    fn heartbeat_to_api_json(&self, node: &Node) -> serde_json::Value {
        ...
        for field in &[
            "category", "project", "branch", "language", "dependencies",
            "machine_name_id", "ai_session", "ai_subscription_plan",
        ] {
            if let Some(v) = node.get_field(&format!("wakatime:{}", field)) {
                match v {
                    FieldValue::String(s) => {
        ...
        for field in &[
            "project_root_count", "ai_line_changes", "human_line_changes",
            "ai_input_tokens", "ai_output_tokens", "ai_prompt_length",
            "lines", "lineno", "cursorpos",
        ] {
            if let Some(v) = node.get_field(&format!("wakatime:{}", field)) {
                match v {
        ...
        if let Some(FieldValue::Boolean(b)) = node.get_field("wakatime:is_write") {
    ...
    async fn rollup_daily_summaries(
        &self,
        ctx: &dyn PluginContext,
    ) -> Result<(), PluginError> {
        ...
        for node in &nodes {
            ...
            let duration = node_duration_seconds(&node);
            let is_ai = match node.get_field("wakatime:is_ai_generated") {
                Some(FieldValue::Boolean(b)) => *b,
                _ => false,
            };
            let human_lines = match node.get_field("wakatime:human_line_changes") {
        ...
        for (key, (total_secs, hb_count, _, ai_secs, lines_added, lines_removed)) in &groups {
            ...
            node.set_field("wakatime:date", FieldValue::String(yesterday.clone()));
            node.set_field("wakatime:project", FieldValue::String(parts[1].to_string()));
            node.set_field("wakatime:language", FieldValue::String(parts[2].to_string()));
            node.set_field("wakatime:entity", FieldValue::String(parts[3].to_string()));
            node.set_field("wakatime:category", FieldValue::String(parts[4].to_string()));
            node.set_field("wakatime:total_seconds", FieldValue::Float(*total_secs));
            node.set_field(
                "wakatime:heartbeat_count",
                FieldValue::Integer(*hb_count as i64),
            );
            node.set_field(
                "wakatime:session_count",
                FieldValue::Integer(session_count as i64),
            );
            node.set_field("wakatime:ai_seconds", FieldValue::Float(*ai_secs));
            node.set_field("wakatime:lines_added", FieldValue::Integer(*lines_added));
            node.set_field("wakatime:lines_removed", FieldValue::Integer(*lines_removed));
...
mod tests {
    ...
    fn test_heartbeat_is_ai() {
        assert!(heartbeat_is_ai(&serde_json::json!({"ai_session": "sess-123"})));
        assert!(heartbeat_is_ai(&serde_json::json!({"ai_line_changes": 5})));
        assert!(heartbeat_is_ai(&serde_json::json!({"ai_input_tokens": 100})));
        assert!(!heartbeat_is_ai(&serde_json::json!({"entity": "/src/main.rs"})));
    }
}
