// Wakatime Plugin — Coding Activity Dashboard
// Shows project and language leaderboards with stats from the wakatime plugin.
...
export default function WakatimeApp({ pluginId }: WakatimeAppProps) {
  const PID = pluginId || 'io.mzhang.panorama.wakatime'
  ...
  const fetchAllStats = async () => {
    ...
    try {
      const [projRes, langRes, fileRes, tsRes] = await Promise.all([
        callPlugin(PID, 'stats', 'GET', null, { range: timeRange, group_by: 'project', aggregation: 'leaderboard' }),
        callPlugin(PID, 'stats', 'GET', null, { range: timeRange, group_by: 'language', aggregation: 'leaderboard' }),
        callPlugin(PID, 'stats', 'GET', null, { range: timeRange, group_by: 'entity', aggregation: 'leaderboard' }),
        callPlugin(PID, 'stats', 'GET', null, { range: timeRange, group_by: 'project', aggregation: 'timeseries', bucket: 'day' }),
      ])
  ...
  const renderTimeseries = () => {
    ...
    const W = 700, H = 200, PAD = { top: 12, right: 12, bottom: 30, left: 50 }
    const plotW = W - PAD.left - PAD.right
    const plotH = H - PAD.top - PAD.bottom

    const allPoints = timeseriesData.flatMap(s => s.data.map(d => ({ ...d, key: s.key })))
      ...
      <div className="card" style={{ padding: 16 }}>
        ...
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', minHeight: 180 }}>
          ...
          {[0, 0.25, 0.5, 0.75, 1].map(pct => {
            ...
            return (
              <g key={pct}>
                ...
                <text x={PAD.left - 6} y={y + 4} textAnchor="end" fill="var(--text-muted)" fontSize={10}>
          ...
          {timeseriesData.map((series, si) => {
            ...
            return (
              <g key={series.key}>
                <path d={`${path} L ${toX(sorted[sorted.length - 1].time)} ${toY(0)} L ${toX(sorted[0].time)} ${toY(0)} Z`}
        ...
        <div style={{ display: 'flex', gap: 12, marginTop: 8, flexWrap: 'wrap', fontSize: 12 }}>
          {timeseriesData.map((series, si) => (
            <div key={series.key} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
              <span style={{ width: 10, height: 10, borderRadius: 2, background: COLORS[si % COLORS.length] }} />
              {series.key}
            </div>
          ))}
        </div>
