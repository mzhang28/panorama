...
impl ObjectStorage {
            ...
            for file in files.flatten() {
              ...
              if name.ends_with(".meta.json") {
                if let Ok(content) = std::fs::read_to_string(file.path()) {
                  if let Ok(meta) = serde_json::from_str::<ObjectMetadata>(&content) {
  ...
  pub fn list(&self, bucket: &str, prefix: Option<&str>) -> Result<Vec<ObjectRef>, String> {
  ...
  pub fn upload_chunk(
    ...
    let mut upload = self.uploads.get_mut(upload_id).ok_or("Upload not found")?;
  ...
  pub fn complete_upload(&self, upload_id: &Uuid) -> Result<ObjectRef, String> {
    let upload = self.uploads.remove(upload_id).ok_or("Upload not found")?.1;
  ...
  pub fn list_buckets(&self) -> Vec<BucketInfo> {
    ...
    for entry in self.objects.iter() {
      let meta = entry.value();
      let info = buckets.entry(meta.bucket.clone()).or_insert((0, 0));
      info.0 += 1;
      info.1 += meta.size;
    }
    buckets
