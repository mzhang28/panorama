...
impl MetaStore {
  ...
  pub fn get_namespace(
    ...
    let mut stmt = conn
      .prepare("SELECT ns_id, kind, app_id, stable_identifier FROM namespaces WHERE ns_id = ?1")?;
  ...
  pub fn list_namespaces(conn: &Connection) -> Result<Vec<Namespace>, rusqlite::Error> {
    let mut stmt = conn
      .prepare("SELECT ns_id, kind, app_id, stable_identifier FROM namespaces ORDER BY ns_id")?;
  ...
  pub fn get_schema_table(
    conn: &Connection,
    schema_id: &Uuid,
  ) -> Result<Option<SchemaTable>, rusqlite::Error> {
    let mut stmt = conn.prepare(
  ...
  pub fn physical_table_for_schema(
    conn: &Connection,
    schema_id: &Uuid,
  ) -> Result<Option<String>, rusqlite::Error> {
    let mut stmt =
      conn.prepare("SELECT physical_table_name FROM schema_tables WHERE schema_id = ?1")?;
    let mut rows = stmt.query_map(params![schema_id.to_string()], |row| {
      row.get::<_, String>(0)
    })?;
    match rows.next() {
  ...
  pub fn get_nodes_with_field_any_ns(
    ...
    let mut stmt =
      conn.prepare("SELECT DISTINCT node_id FROM field_presence WHERE field_name = ?1")?;
  ...
  pub fn get_nodes_with_field(
    ...
    let mut stmt =
      conn.prepare("SELECT node_id FROM field_presence WHERE ns_id = ?1 AND field_name = ?2")?;
  ...
  pub fn remove_schema_conformance(
    conn: &Connection,
    node_id: &Uuid,
  ) -> Result<(), rusqlite::Error> {
    conn.execute(
  ...
  pub fn get_conforming_nodes(
    ...
    if let Some(vmin) = version_min {
      ...
      let mut stmt =
        conn.prepare("SELECT node_id FROM node_schema_conformance WHERE schema_id = ?1")?;
...
mod tests {
  ...
  fn test_system_namespaces_present() {
    let conn = test_conn();
    let sys = MetaStore::get_namespace(&conn, SYSTEM_NS_ID)
      .unwrap()
      .unwrap();
    assert_eq!(sys.kind, NamespaceKind::System);
    assert_eq!(sys.stable_identifier, "system");

    let usr = MetaStore::get_namespace(&conn, USER_NS_ID)
      .unwrap()
      .unwrap();
  ...
  fn test_upsert_schema_table() {
    ...
    let schema_id = Uuid::new_v4();
    let field_mappings =
      serde_json::json!({"title": {"column": "title", "type": "String", "indexed": false}});
  ...
  fn test_field_stats_batch() {
    let conn = test_conn();
    MetaStore::record_field_stats_batch(&conn, &[(1, "node_title", 5, 1, 2)]).unwrap();

    let stats = MetaStore::get_field_stats(&conn, 1, "node_title")
      .unwrap()
      .unwrap();
