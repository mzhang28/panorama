...
use panorama_core::reactor::{
    ActionKind, DeadLetter, DeferredReactorState, OpStreamEntry, OpType, Reactor,
...
impl DeferredReactorEngine {
    ...
    pub async fn initialize(&self) -> Result<(), String> {
        ...
        let query = "MATCH (n) IN space(\"default\") WHERE n CONFORMS TO schema(\"ReactorState\") RETURN n";
        let rows = self
            .registry
            .storage()
            .query_lang(query)
    ...
    pub async fn poll(&self) -> Result<usize, String> {
        ...
        let all_reactors: Vec<Reactor> = self
            ...
            .list_all()
            .into_iter()
            .filter(|r| r.mode == panorama_core::reactor::ReactorMode::Deferred
            ...
            for entry in &entries {
                ...
                match self.execute_action(reactor, entry).await {
                    ...
                    Err(e) => {
                        ...
                        let should_dead_letter = self
                            .handle_failure(reactor, entry, &e)
                            .await;
                ...
                WatchTrigger::LifecycleWatch { event, scope } => {
                    ...
                    let event_matches = match event {
                        ...
                        LifecycleEvent::NodeDeleted => {
                            entry.op_type == OpType::NodeDeleted
                        }
                        LifecycleEvent::SchemaInstalled => {
                            entry.op_type == OpType::SchemaInstalled
                        }
                        LifecycleEvent::SchemaMigrated => {
                            entry.op_type == OpType::SchemaMigrated
                        }
                        LifecycleEvent::AppInstalled => {
                            entry.op_type == OpType::AppInstalled
                        }
                        LifecycleEvent::AppUninstalled => {
                            entry.op_type == OpType::AppUninstalled
                        }
                    };
                    if !event_matches {
                        return false;
    ...
    fn scope_matches(&self, scope: &WatchScope, entry: &OpStreamEntry) -> bool {
        match scope {
            WatchScope::SchemaId(schema_id) => {
                entry.schema_id == Some(*schema_id)
            }
            WatchScope::SpaceId(space_id) => {
        ...
        match reactor.action_kind {
            ActionKind::ComputeField => {
                tracing::debug!(
                    reactor_id = %reactor.id,
                    sequence = entry.sequence,
                    "Executing deferred compute_field action"
                );
            ...
            ActionKind::SideEffect => {
                tracing::debug!(
                    reactor_id = %reactor.id,
                    sequence = entry.sequence,
                    "Executing deferred side_effect action"
                );
                // Side effects (webhooks, emails) would go here.
                // Requires network capability — checked at registration.
                let _ = exec_ctx;
                Ok(())
            }
            ActionKind::InternalWrite => {
                tracing::debug!(
                    reactor_id = %reactor.id,
                    sequence = entry.sequence,
                    "Executing deferred internal_write action"
                );
                // Internal writes would go here, creating/updating nodes
                // non-transactionally with eventual consistency.
                let _ = exec_ctx;
                Ok(())
            }
            _ => Err(format!(
                "Invalid action kind {:?} for deferred reactor",
                reactor.action_kind
            )),
        }
    }
    ...
    async fn handle_failure(
        ...
        if state.consecutive_failures > retry_policy.max_retries {
            ...
            let backoff = (retry_policy.base_backoff_ms as f64
                * retry_policy.backoff_multiplier.powi(
                    state.consecutive_failures as i32 - 1,
                ))
