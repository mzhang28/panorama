...
impl OpStream {
    ...
    pub async fn initialize(&self) -> Result<(), String> {
        let query = "MATCH (n) IN space(\"default\") WHERE n CONFORMS TO schema(\"OpStream\") RETURN n ORDER BY n.system.op_sequence DESC LIMIT 1";
        let rows = self.storage.query_lang(query).map_err(|e| {
            format!("Failed to query op stream: {}", e)
        })?;

        if let Some(row) = rows.first() {
            if let Some(max_seq) = row
                .get("system:op_sequence")
                .and_then(|v| v.as_i64())
            {
                self.next_sequence
    ...
    pub fn append_sync(
        ...
        node.set_field(
            "system:op_sequence",
            FieldValue::Integer(sequence as i64),
        );
        node.set_field(
            "system:op_type",
        ...
        node.set_field(
            "system:op_committed_at",
            FieldValue::DateTime(committed_at),
        );

        self.storage.create(node).map_err(|e| {
            format!("Failed to persist op stream entry: {}", e)
        })?;
    ...
    pub fn query_since(&self, since_sequence: u64, limit: u64) -> Result<Vec<OpStreamEntry>, String> {
        let query = format!(
            "MATCH (n) IN space(\"default\") \
             WHERE n CONFORMS TO schema(\"OpStream\") \
             AND n.system.op_sequence > {} \
             ORDER BY n.system.op_sequence ASC \
             LIMIT {} \
             RETURN n",
        ...
        let entries: Vec<OpStreamEntry> = rows
            .iter()
    ...
    fn row_to_entry(&self, row: &serde_json::Value) -> Option<OpStreamEntry> {
        ...
        // Parse op_data JSON
        let (new_value, previous_value, authorized_by, source_app_id) = match node
            .get_field("system:op_data")
        {
            Some(panorama_core::types::FieldValue::Json(j)) => {
                let new_val = j.get("new_value").and_then(|v| {
                    if v.is_null() { None } else { Some(v.clone()) }
                });
                let prev_val = j.get("previous_value").and_then(|v| {
                    if v.is_null() { None } else { Some(v.clone()) }
                });
                let auth = j
                    .get("authorized_by")
