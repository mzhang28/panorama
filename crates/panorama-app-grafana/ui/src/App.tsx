// Grafana Dashboard Plugin — Full dashboard viewer with panel renderers,
...
const PLUGIN_ID = 'io.mzhang.panorama.grafana'
const WAKA_PLUGIN = 'io.mzhang.panorama.wakatime'
...
interface DashboardTime { from: string; to: string }
...
const PALETTE = [
  '#6c8cff', '#ff8c6c', '#6cff8c', '#8c6cff',
  '#ff6c8c', '#8cff6c', '#ffcc6c', '#6cc8ff',
  '#c86cff', '#6cffcc', '#ff6ccc', '#ccff6c',
  '#ff9966', '#66b3ff', '#ff66b3', '#b3ff66',
]
...
function TimeRangePicker({
  ...
  time: DashboardTime; onChange: (t: DashboardTime) => void;
}) {
  const [custom, setCustom] = useState(false)

  const activePreset = PRESETS.find(p => p.from === time.from && p.to === time.to)
...
function LeaderboardPanel({ data, options }: { data: DataFrame[]; options?: Record<string, unknown> }) {
  ...
  if (!displayed.length) {
    return <p className="text-muted" style={{ padding: 16, textAlign: 'center' }}>No data</p>
  }
...
function TimeseriesPanel({ data, options }: { data: DataFrame[]; options?: Record<string, unknown> }) {
  const W = 700, H = 240, PAD = { top: 16, right: 16, bottom: 32, left: 56 }
  ...
  if (!allSeries.length) {
    return <p className="text-muted" style={{ padding: 16, textAlign: 'center' }}>No time-series data</p>
  }
  ...
  // Y-axis ticks
  const yTicks = 4
  const yTickVals = Array.from({ length: yTicks + 1 }, (_, i) => vMin + (vRange * i) / yTicks)
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 'auto', minHeight: 200 }}>
      ...
      {/* X-axis labels */}
      {xTickVals.map((t, i) => (
        <text key={`x-${i}`} x={toX(t)} y={H - 6} textAnchor="middle"
          fill="var(--text-muted)" fontSize={10}>
      ...
      {allSeries.map((series, si) => {
        ...
        const pathD = sorted.map((p, i) =>
          `${i === 0 ? 'M' : 'L'} ${toX(p.time)} ${toY(p.val)}`
        ).join(' ')
        const areaD = pathD + ` L ${toX(sorted[sorted.length - 1].time)} ${toY(0)} L ${toX(sorted[0].time)} ${toY(0)} Z`
        ...
        return (
          <g key={series.key}>
            ...
            <path d={pathD} fill="none" stroke={color} strokeWidth={options?.lineWidth as number ?? 2}
...
function StatPanel({ data, options }: { data: DataFrame[]; options?: Record<string, unknown> }) {
  const rows = data[0]?.rows
  const val = rows?.[0]?.[0]
...
/** Pie chart: SVG donut */
function PieChartPanel({ data, options }: { data: DataFrame[]; options?: Record<string, unknown> }) {
  const rows = data[0]?.rows ?? []
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        {rows.slice(0, 8).map((row: any[], i: number) => (
          <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3, fontSize: 12 }}>
            ...
            <span className="text-muted" style={{ marginLeft: 'auto', fontFamily: 'monospace', fontSize: 11 }}>
...
/** Table panel */
function TablePanel({ data, options }: { data: DataFrame[]; options?: Record<string, unknown> }) {
  const frame = data[0]
...
const thStyle: CSSProperties = {
  ...
  fontWeight: 600, position: 'sticky', top: 0, background: 'var(--bg-card)',
}
...
function HeatmapPanel({ data }: { data: DataFrame[] }) {
  ...
  const dayMap = new Map(rows.map((r: any[]) => [String(r[0]), Number(r[2] ?? 0)]))
    ...
    <div style={{ padding: 8, overflow: 'auto' }}>
      <div style={{ display: 'flex', gap: 3 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 3, marginRight: 4 }}>
          {dayNames.map((name, i) => (
            <div key={i} style={{ width: 24, height: 14, fontSize: 9, lineHeight: '14px', textAlign: 'right' }}
              className="text-muted">{name}</div>
          ))}
        </div>
        {/* Weeks */}
        {weeks.map((week, wi) => (
      ...
      <div style={{ display: 'flex', gap: 4, alignItems: 'center', marginTop: 8, fontSize: 10 }}
        className="text-muted">
        <span>Less</span>
        {[0, 0.25, 0.5, 0.75, 1].map(pct => (
          <div key={pct} style={{ width: 12, height: 12, borderRadius: 2, background: getIntensity(maxHours * pct) }} />
        ))}
        <span>More</span>
      </div>
    </div>
...
export default function GrafanaApp({ pluginId }: { pluginId: string }) {
  const pid = pluginId || PLUGIN_ID
  const [dashboards, setDashboards] = useState<any[]>([])
  const [currentDashboard, setCurrentDashboard] = useState<Dashboard | null>(null)
  const [panelData, setPanelData] = useState<Map<number, DataFrame[]>>(new Map())
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')
  const [showBuilder, setShowBuilder] = useState(false)
  const [editingDashboard, setEditingDashboard] = useState<Dashboard | null>(null)
  const [time, setTime] = useState<DashboardTime>({ from: 'now-7d', to: 'now' })
  ...
  useEffect(() => {
    ...
    if (!secs) return
    const timer = setInterval(() => fetchPanelData(currentDashboard), secs * 1000)
  ...
  const handleSaveDashboard = async (dash: Dashboard) => {
    try {
      ...
      const endpoint = dash.uid ? `api/dashboards/${dash.uid}` : 'api/dashboards'
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '0 16px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 8 }}>
        <h2 style={{ margin: 0 }}>
          {currentDashboard?.title ?? 'Dashboards'}
        </h2>
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <button className="primary" onClick={() => {
            setEditingDashboard(currentDashboard ? { ...currentDashboard } : {
              uid: '', title: 'New Dashboard', time: { from: 'now-7d', to: 'now' },
              panels: [],
            })
            setShowBuilder(true)
          }}>
      ...
      {error && (
        <div className="card" style={{ marginBottom: 16, color: 'var(--danger)', padding: 12 }}>
          {error}
          <button style={{ marginLeft: 12, fontSize: 12 }} onClick={() => setError('')}>Dismiss</button>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(24, 1fr)', gap: 12 }}>
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                  ...
                  {panel.description && (
                    <span className="text-muted" style={{ fontSize: 11 }} title={panel.description}>ⓘ</span>
                  )}
                </div>
      ...
      {showBuilder && editingDashboard && (
        <DashboardBuilder
          ...
          onDelete={editingDashboard.uid ? () => handleDeleteDashboard(editingDashboard.uid) : undefined}
        />
      )}
    </div>
  )
...
function DashboardBuilder({
  dashboard, onChange, onSave, onCancel, onDelete,
}: {
  dashboard: Dashboard; onChange: (d: Dashboard) => void;
  onSave: (d: Dashboard) => void; onCancel: () => void; onDelete?: () => void;
}) {
  ...
  const handleUpdatePanel = (id: number, updates: Partial<Panel>) => {
    onChange({
      ...dashboard,
      panels: dashboard.panels.map(p => p.id === id ? { ...p, ...updates } : p),
    })
  }

  const handleRemovePanel = (id: number) => {
    onChange({ ...dashboard, panels: dashboard.panels.filter(p => p.id !== id) })
  }
  ...
  return (
    <div style={{
      ...
      <div className="card" style={{
        ...
        <h3 style={{ marginTop: 0 }}>{dashboard.uid ? 'Edit Dashboard' : 'New Dashboard'}</h3>

        {/* Dashboard meta */}
        <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 200 }}>
            <label className="text-muted" style={{ display: 'block', marginBottom: 4 }}>Title</label>
            <input
              type="text" value={dashboard.title}
              onChange={e => onChange({ ...dashboard, title: e.target.value })}
              style={{ width: '100%' }}
            />
          </div>
          <div style={{ flex: 1, minWidth: 200 }}>
            <label className="text-muted" style={{ display: 'block', marginBottom: 4 }}>Description</label>
            <input
              type="text" value={dashboard.description ?? ''}
              onChange={e => onChange({ ...dashboard, description: e.target.value })}
              style={{ width: '100%' }}
            />
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 12, marginBottom: 16, flexWrap: 'wrap' }}>
          <div>
            <label className="text-muted" style={{ display: 'block', marginBottom: 4 }}>Time From</label>
            <input
              type="text" value={dashboard.time?.from ?? 'now-7d'}
              onChange={e => onChange({ ...dashboard, time: { ...dashboard.time, from: e.target.value } })}
              style={{ width: 120 }}
            />
          </div>
          <div>
            <label className="text-muted" style={{ display: 'block', marginBottom: 4 }}>Time To</label>
            <input
              type="text" value={dashboard.time?.to ?? 'now'}
              onChange={e => onChange({ ...dashboard, time: { ...dashboard.time, to: e.target.value } })}
              style={{ width: 120 }}
            />
          </div>
          <div>
            <label className="text-muted" style={{ display: 'block', marginBottom: 4 }}>Refresh</label>
            <select value={dashboard.refresh ?? ''}
              onChange={e => onChange({ ...dashboard, refresh: e.target.value })}>
              <option value="">Off</option>
              <option value="30s">30s</option>
              <option value="1m">1m</option>
              <option value="5m">5m</option>
        ...
        <div>
          ...
          {dashboard.panels.map((panel, idx) => (
            <div key={panel.id} className="card" style={{ marginBottom: 12, padding: 12, background: 'var(--bg)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                <strong>Panel {panel.id}: {panel.title}</strong>
                <button
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <div>
                  <label className="text-muted" style={{ display: 'block', fontSize: 11 }}>Title</label>
                  <input type="text" value={panel.title}
              ...
              {panel.queries.map((q, qi) => (
                <div key={qi} style={{ marginTop: 8, padding: '8px', background: 'var(--bg-card)', borderRadius: 4 }}>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'end' }}>
                    <div style={{ minWidth: 100 }}>
                      <label className="text-muted" style={{ display: 'block', fontSize: 11 }}>Data Source</label>
                      <input type="text" value={q.data_source}
                        onChange={e => {
                          const queries = [...panel.queries]
                          queries[qi] = { ...queries[qi], data_source: e.target.value }
                    ...
                    <div>
                      ...
                      <input type="number" value={q.limit ?? 10}
                        onChange={e => {
                          const queries = [...panel.queries]
                          queries[qi] = { ...queries[qi], limit: parseInt(e.target.value) || 10 }
                          handleUpdatePanel(panel.id, { queries })
                        }}
              ...
              <details style={{ marginTop: 8, fontSize: 12 }}>
                <summary className="text-muted" style={{ cursor: 'pointer' }}>Grid Position</summary>
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  {(['x', 'y', 'w', 'h'] as const).map(k => (
                    <div key={k}>
                      <label className="text-muted" style={{ display: 'block', fontSize: 10 }}>{k}</label>
                      <input type="number" value={panel.grid_pos?.[k] ?? (k === 'w' ? 12 : k === 'h' ? 8 : 0)}
                        onChange={e => {
                          const gp = { ...panel.grid_pos, [k]: parseInt(e.target.value) || 0 }
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 20, justifyContent: 'flex-end' }}>
          {onDelete && (
            <button style={{ color: 'var(--danger)', marginRight: 'auto' }} onClick={onDelete}>
              Delete Dashboard
            </button>
          )}
