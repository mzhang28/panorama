// Grafana / Dashboards Plugin — React UI component
// Time-series data visualization with leaderboard tables.
...
export default function GrafanaApp({ pluginId }: GrafanaAppProps) {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const res = await callPluginEndpoint(PLUGIN_ID, 'query', 'POST', {
        group_by: groupBy,
        aggregation,
        filter: '',
      })
      ...
      const dashRes = await callPluginEndpoint(PLUGIN_ID, 'dashboards')
      ...
      <div className="card" style={{ marginBottom: 20 }}>
        <div className="flex-row" style={{ gap: 12, flexWrap: 'wrap' }}>
          <div>
            ...
            >
              {queryOptions.map((o) => (
                <option key={o.groupBy} value={o.groupBy}>
                  {o.label}
                </option>
              ))}
            </select>
                ...
                >
                  ...
                  >
                    {aggregation === 'leaderboard'
                      ...
                      : item.count ?? item.total_duration_seconds ?? '-'}
                  </td>
                </tr>
              ))}
