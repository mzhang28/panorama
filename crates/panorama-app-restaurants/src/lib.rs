...
impl RestaurantsPlugin {
    ...
    async fn compute_rankings(
        ...
        let restaurant_rows = ctx
            .query("MATCH (n) IN space(\"default\") WHERE HAS_FIELD(n, \"restaurants\", \"cuisine\") RETURN n")
            .await?;
    ...
    async fn handle_http_request(
        ...
        match (request.method.as_str(), endpoint) {
            ...
            ("POST", "compare") => {
                ...
                let mut node = Node::new(Uuid::nil());
                node.set_field("restaurants:better_id", FieldValue::String(better.to_string()));
                node.set_field("restaurants:worse_id", FieldValue::String(worse.to_string()));
                if let Some(ctx_str) = body["context"].as_str() {
                    node.set_field("restaurants:context", FieldValue::String(ctx_str.to_string()));
                }
